...
Grunden til at der kun laves et proof of concept og ikke et fuldstændigt færdigt system er at der er ikke nok tid til at gøre det helt færdigt. 
...
Denne sektion om proof of concept beskriver valg af sensor data som er nødvendigt, hvordan søvnestimering udføres på denne data, og hvordan søvn estimeringerne for de forskellige sensorer kombineres og hvordan denne estimeringen skal aggregeres til individuelle perioder af søvn.
...
Metoden er baseret på redskaberne beskrevet i \cref{sec:redskaber}, hvor vi som start ved dataindsamlingsmoduler ser på amplitude og acceleration.
Grunden til at vi starter med disse to datakilder er at \citet{6563918} erfarede at det var de langt væsentligste datakilder, også beskrevet i \cref{sec:BES}.

Disse to datakilder kombineres til en enkelt model der skal kunne vurderer hvorvidt man sover eller ej.
Med mere tid vil de andre nævnte datakilder også blive fokuseret på.

For at have en regelmæssig måde at indsamle data, der ligger til grund for implementationen af det efterfølgende søvnestimeringsmodul, er det nødvendigt at have forsøgsopstillingen beskrevet, hvorfor denne følger herefter.
...
Idéen er at det eneste krav til indsamlingen er at den skal ligge på et bord i soveværelset når der soves, ellers står det en frit om man går med telefonen i lommen, ligger den på skrivebordet eller andet. Dette valg er foretaget i henhold til vores kriterie om at udvikle et non-intrusive system.
...
Ved videre arbejde bør en mere systematisk tilgang med den nævnte krydsvalidering benyttes og er beskrevet videre i \cref{sec:verisoevn}.
...
Som nævnt har vi ikke kunnet finde passende metoder der er offentligt tilgængeligt, hvorfor vi bliver nødt til at udvikle vores egen algoritme.
...
Konceptet om stilstand er taget fra \citet{6563918} angående Best Effort Model, og er hvilken model vores algoritme tager primært udgangspunkt i.
Toss 'N' Turns tilgang hvor man ser på 10-minutters vinduer med data til at afgøre søvn eller ikke søvn er et alternativ.
Dette alternativ blev dog valgt fra i første omgang, da stilstandsbestemmelsen vurderes mindre komplekst og egnede sig derfor bedre til en proof of concept løsning.
...
Vores implementering af det vægtede gennemsnit afviger fra et normalt vægtet gennemsnit da vi har data der ikke er optaget på samme tid og heller ikke data i samme mængder for de enkelte søvnestimerings moduler.
...
\als{Kan være vi skal justere figurerne så vi har "fortællingen" fra data til modeller til kombinering}
